...
function initLegend() {
  ...
  moveEl.innerHTML = `
    <li>
      ...
      <span>Move path (solid neon green)</span>
    </li>
    <li>
      <div class="legend-sample legend-sample--move" aria-hidden="true">
        <span class="move-squad-label move-squad-label--legend">Able</span>
      </div>
      <span>Squad name on the line</span>
    </li>
    <li>
      <div class="legend-sample legend-sample--move" aria-hidden="true">
        ${routeArrowSvg(null, "route-arrow--legend")}
      </div>
      <span>Arrow at end — direction of travel</span>
    </li>
    <li>
      <span class="legend-sample legend-sample--scroll">⟳</span>
...
function routeArrowSvg(angleDeg, extraClass = "") {
  ...
  return `<svg class="${cls}" viewBox="0 0 20 26"${style} aria-hidden="true"><polygon class="route-arrow__shape" points="10,1 18,25 2,25"/></svg>`;
}
...
function routeArrowAngleDeg(obj) {
  if (obj.arrowAngleDeg != null) return obj.arrowAngleDeg;
...
function rotateRouteArrow(routeId, deltaDeg) {
  ...
  obj.arrowAngleDeg = routeArrowAngleDeg(obj) + deltaDeg;
  setArrowMarkerAngle(obj.arrowMarker, obj.arrowAngleDeg);
}
  ...
  container.addEventListener(
    ...
    (e) => {
      if (drawing && temp) {
        ...
        drawArrowAdjusted = true;
        drawArrowAngleDeg += wheelRotateDelta(e);

        const latlngs = temp.getLatLngs();
      ...
      blockMapWheelZoom(e);
      rotateRouteArrow(routeId, wheelRotateDelta(e));

      clearTimeout(wheelRouteSaveTimer);
      wheelRouteSaveTimer = setTimeout(() => {
...
function setupRouteDecorations(id) {
  ...
  const start = pairToLatLng(obj.points[0]);
  const end = pairToLatLng(obj.points[obj.points.length - 1]);
  const mid = routeMidpoint(obj.points);
  ...
  obj.squadMarker = createMoveSquadLabel(mid, obj.squad);
  obj.squadMarker.addTo(map);
  bindDeleteClick(obj.squadMarker, id);
  ...
  cleaned.forEach((m) => {
    ...
    if (m.type === "route") {
      ...
      objects[id] = {
        ...
        hitLine,
        points: m.points,
        arrowAngleDeg: m.arrowAngleDeg ?? lineAngleDeg(routeStart, routeEnd),
        startHandle: null,
        arrowMarker: null,
        squadMarker: null
      };
